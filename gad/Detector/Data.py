...
class MEM_DiskFile(Data):
    ...
    def get_min_max(self, feas):
        # ar = self.get_rows(feas)
        # ar_view = ar.view((ar.dtype[0], len(ar.dtype.names)))
        # return ar_view.min(axis=0), ar_view.max(axis=0)
        min_vec = []
        max_vec = []
        for fea in feas:
            ar = self.get_rows([fea])
            ar_view = ar.view((ar.dtype[0], len(ar.dtype.names)))
...
class CSVFile(MEM_DiskFile):
    ...
    def get_rows(self, fields=None, rg=None, rg_type=None):
        if fields is None:
            fields = list(self.FIELDS)
        sp, ep = self.get_where(rg, rg_type)
        return self.table[sp:ep][fields].as_matrix()

class MEM_IPFile(MEM_DiskFile):
    def _init(self):
        ...
        self.cluster_map, self.dist_to_center_map = \
            self._cluster_src_ip(fea_option['cluster'][0], self.unique_ip)
        cluster = [self.cluster_map[ip] for ip in self.ip]
        dist_to_center = [self.dist_to_center_map[ip] for ip in self.ip]
        self.table = recfunctions.append_fields(self.table, ['cluster', 'dist_to_center'],
    ...
    def _cluster_src_ip(self, cluster_num, unique_ip):
        ...
        dist_to_center_map = dict(zip(unique_ip, dist_to_center))
        return cluster_map, dist_to_center_map

    def get_IP(self, rg, rg_type):
        src_ip_int_vec_tmp = self.get_rows(['src_ip'], rg, rg_type)
        src_ip_vec = [tuple(x[0]) for x in src_ip_int_vec_tmp]
        return src_ip_vec


class MEM_FS(MEM_IPFile):
